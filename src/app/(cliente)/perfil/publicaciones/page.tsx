...
import { ArrowLeft, Clock, CheckCircle, XCircle, Eye, Loader2, Trash2, Package, Edit3 } from "lucide-react"
...
const STATUS_CONFIG: Record<string, { icon: typeof Clock; label: string; className: string }> = {
  ...
  changes_requested: { icon: Edit3, label: "Cambios solicitados", className: "bg-warning-50 text-warning-600" },
...
export default function PublicacionesPage() {
  ...
  const cambiosSolicitados = publicaciones.filter(p => p.status === "changes_requested")
            ...
            {cambiosSolicitados.length > 0 && (
              <section>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="text-sm font-semibold text-text-strong">Cambios solicitados</h2>
                  <span className="text-xs text-text-muted">({cambiosSolicitados.length})</span>
                </div>
                <div className="space-y-3">
                  {cambiosSolicitados.map(p => (
                    <div key={p.id}
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between">
                          <div>
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-warning-50 text-warning-600 shrink-0">
                                Cambios
                              </span>
                            </div>
